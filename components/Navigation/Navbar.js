...
export default function Navbar() {
    ...
    return routes.map((route) => {
      return (
        <Link key={route.id} href={route.path}>
          <a className="hvr-grow text-base font-normal text-gray-700 dark:text-white">
            {route.name}
  ...
  return (
    <div id="top" className="max-w-6xl  mx-auto px-4 py-2 md:py-6">
      <div className="flex md:flex-row justify-between items-center">
        <div className="flex flex-col">
          <Link href="/">
            <a>
              <h1 className="hvr-pulse font-semibold text-xl text-gray-800 dark:text-white">
                {data.name}
              </h1>
        ...
        <div className="space-x-8 hidden md:block">{generateRoutes()}</div>
      ...
      <div className="space-x-8 block md:hidden mt-4">{generateRoutes()}</div>
    </div>
  );
}
